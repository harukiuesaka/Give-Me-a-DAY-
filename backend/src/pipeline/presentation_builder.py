...
def build_presentation(
    ...
    if recommendation.best_candidate_id:
        ...
        if best:
            cards.append(
                _build_card(best, CandidateLabel.PRIMARY, recommendation, audit_map.get(best.candidate_id))
            )
    ...
    if recommendation.runner_up_candidate_id:
        ...
        if runner_up:
            cards.append(
                _build_card(
                    runner_up,
                    CandidateLabel.ALTERNATIVE,
                    recommendation,
                    audit_map.get(runner_up.candidate_id),
                )
            )

    # Presentation context
    context = _build_context(recommendation, candidates, cards, audits)
...
def _build_card(
    ...
    key_risks = list(audit.residual_risks[:3]) if audit and audit.residual_risks else candidate.known_risks[:3]
    if len(key_risks) < 2:
...
def _build_context(
    ...
    if rejected > 0:
        rejection_headline = _build_rejection_headline(rejected, recommendation.rejected_candidate_ids, audits)
...
def _build_rejection_headline(
    rejected_count: int,
    rejected_candidate_ids: list[str],
    audits: list[Audit] | None,
) -> str:
    if audits:
        audit_map = {audit.candidate_id: audit for audit in audits}
        reasons = [
            audit_map[candidate_id].rejection_reason
            for candidate_id in rejected_candidate_ids
            if candidate_id in audit_map and audit_map[candidate_id].rejection_reason
        ]
        if reasons:
            first_reason = reasons[0].split("。", 1)[0].strip()
            return f"{rejected_count}方向を棄却。主な理由: {first_reason}"

    return f"{rejected_count}方向を棄却。主な理由: 計画段階または実行段階の監査で承認条件を満たさなかった"
